...
class Computations(object):
    ...
    def get_option(self, option, default=None, cast_type=None):
        val = self.context.params.get('config', option, default)
...
class Template(object):
    def __init__(self, context, use_locale=True):
        ...
        self.local_data = {
            'altitude': self.computations.get_option('altitude', cast_type=int),
            'latitude': self.computations.get_option('latitude', cast_type=float),
            'longitude': self.computations.get_option('longitude', cast_type=float)
        }
    ...
    def process(self, live_data, template_file):
        ...
        get_option = self.computations.get_option
        ...
        while True:
            ...
            for i, part in enumerate(parts):
                ...
                if command == []:
                    ...
                    yield u'#'
                elif command[0] in list(data.keys()) + list(self.local_data.keys()) + ['calc']:
                    # output a value
                    if not valid_data:
                        continue
